...
use pricing::simulation::monte_carlo2::{MonteCarloPathSimulator as mcps, DistributionExt, McPathSampler};
...
pub fn criterion_stock_price_simulation(c: &mut Criterion) {
    ...
    group.bench_function(
        ...
        |b| b.iter(|| simulate_paths_with_path_fn(black_box((100_000, 100)))),
    );
    group.bench_function("apply a path function on the stored paths", |b| {
        b.iter(|| simulate_paths_with_path_generator(black_box((100_000, 100))))
    });
    group.bench_function("apply a path function on the stored paths NEW", |b| {
        b.iter(|| simulate_paths_with_path_generator_new(black_box((100_000, 100))))
    });
